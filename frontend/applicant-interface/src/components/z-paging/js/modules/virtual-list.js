  ...
  methods: {
    ...
    _updateFixedCellHeight() {
      this.$nextTick(() => {
        const updateFixedCellHeightTimeout = setTimeout(() => {
          this._getNodeClientRect(`#zp-id-${0}`, this.finalUseInnerList).then(
            (cellNode) => {
              if (!cellNode) {
      ...
      this.$nextTick(() => {
        const updateDynamicCellHeightTimeout = setTimeout(async () => {
          for (let i = 0; i < list.length; i++) {
            ...
            const lastHeightCache = this.virtualHeightCacheList.length
              ? this.virtualHeightCacheList.slice(-1)[0]
              : null;
            const lastHeight = lastHeightCache
    ...
    _setCellIndex(list, isFirstPage) {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        item[c.listCellIndexUniqueKey] = `${this.virtualListKey}-${
          item[c.listCellIndexKey]
        }`;
        list[i] = item;
      }
